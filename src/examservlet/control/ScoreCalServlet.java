	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		for (int i = 0; i < ques.size(); ++i) {
			...
			if (!que.getQ_type().equals("简答题")) {//非简答题的记分方式相同，简答题独立计算
				if (stuAnsArr != null) {
					StringBuilder stuAns = new StringBuilder(); //每道题的答案
                    for (String s : stuAnsArr) {//多选题拥有多个答案
                        stuAns.append(s);//组装学生答案
                    }
					System.out.println(stuAns);
					if (stuAns.toString().equalsIgnoreCase(que.getQ_key())) {	//匹配学生答案和正确答案
                ...
                for (String s : KEY_WORD) {
                    if (stuAns.contains(s)) { //判断考生答案中是否出现关键词
                        System.out.println(stuAns + "包含关键词:" + s);
                        score_ess += singleScore;
                    } else {
                        System.out.println("不包含关键词:" + s);
                    }
                }
		...
		int g = ((f < 0) ? 1 : 0) + ((f < 60) ? 1 : 0) + ((f < 75) ? 1 : 0)
				+ ((f < 85) ? 1 : 0) + ((f < 95) ? 1 : 0);
		switch (g) {
		case 0:
			grade = "优秀";
